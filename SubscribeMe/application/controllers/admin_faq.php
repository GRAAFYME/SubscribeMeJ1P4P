...
class Admin_faq extends AD_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('admin_faq_model'); // Load model(s)
	}
	...
	public function index($offset = 0)
	{
        ...
        // Generate table data
        $this->table->set_heading('id', 'Question', 'Answer', '');
	...
	function update() 
	{
		...
		// Prefill form values
		if ($this->input->post('question') == '')		{	$this->form_validation->question = $entry->question;				}
		else 											{	$this->form_validation->question = $this->input->post('question');	}

		if ($this->input->post('answer') == '')			{	$this->form_validation->answer = $entry->answer;					}
		else 											{	$this->form_validation->answer = $this->input->post('answer');		}
		...
		{
			$bool = $this->admin_faq_model->update($id); // Update function will return true or false in $bool
	...
	{		
		...
		$this->admin_faq_model->delete($id); // Delete this record
